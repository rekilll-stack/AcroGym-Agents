...
doc.pipe(fs.createWriteStream('/tmp/font-test-v4.pdf'));
...
async function buildAndFinalize() {
  ...
  const INSIGHT_Y = LINE_Y + 18 + LINE_PDF_H + 10;
  const ziw = icons.draw(doc, 'Lightbulb', M, INSIGHT_Y, { size: 13, color: ORANGE });
  doc.fontSize(9).fillColor(DARK).font('M-Regular');
  txt(
    noLig(`Best: ${lineLabels[maxIdx]} — ${lineData[maxIdx]} leads.  Worst: ${lineLabels[minIdx]} — ${lineData[minIdx]} lead${lineData[minIdx] !== 1 ? 's' : ''}.`),
    M + ziw, INSIGHT_Y + 1
  );
  ...
  doc.end();
  console.log('font-test-v4.pdf → /tmp/font-test-v4.pdf');
}
